...
def plot_connectivity(n_neurons, exc_synapses, inhib_synapses, connectivity2, centrality, p_in_measured, p_out_measured, percent_central):
...
def print_centrality(C_total, cluster_nodes, p_input, measure_name):
    ...
    if measure_name != 'random':
        ...
        measure_values_cluster = {node: measure_values[node] for node in range(int(len(cluster_list)/2))}
...
def sim(p_within, p_between, g, nu_ext_over_nu_thr, J, J2, refractory_period, sim_time, plotting_flags,
    ...
    # centrality = filter(lambda x: x < N1, centrality)
    ...
    if plotting_flags.get('connectivity2', False):
        ...
        plot_connectivity(n_neurons, exc_synapses, inh_synapses, connectivity2, centrality, p_in_measured, p_out_measured, percent_central)
