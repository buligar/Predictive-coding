...
def sim(p_within, p_between, g, nu_ext_over_nu_thr, J, sim_time, ax_spikes, ax_rates, ax_rates2, ax_trace,
    ...
    # Подключаем все возможные возбуждающие синапсы
    exc_synapses.connect(condition='i != j and i < N_E')
    # Подключаем все возможные тормозные синапсы
    ...
    ax_rates.set_title('Частота спайков neu.0-50', fontsize=16)
    ax_rates2.set_title('Частота спайков neu.50-100', fontsize=16)
    ...
    ax_spectrogram.set_title('Глобальные частоты нейронов 1 и 2 кластера', fontsize=16)
...
for p_within_idx, p_within in enumerate(p_within_values):
    ...
    imageio.mimsave(f'gif_exc_inh_p_within_{p_within:.2f}.gif', images, duration=3000, loop=0)
...
plt.savefig('average_spike_dependency_in_2_cluster_ext_inh.png')
plt.show()
