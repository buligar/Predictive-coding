...
p_within_values = np.arange(0.2, 0.21, 0.05)
...
boost_factor_list = [2, 100, 20, 50, 50, 30, 0]
...
def print_centrality(C_total, N1, cluster_nodes, p_input, boost_factor, measure_name, direct_1_2=True):
    """
...
def plot_pinput_between_avg_spikes_with_std(
    ...
    """
    Для КАЖДОЙ метрики measure_name (кроме 'random') строит 3D-график, 
    накладывая на него вторую поверхность для 'random'.
    
    Структура хранения данных:
      spike_counts_second_cluster_for_input[I0_value][measure_name][p_within][p_input][p_between] = mean_spikes
      spike_counts_second_cluster[I0_value][measure_name][p_within][p_between] = список, 
         где каждая запись — это массив средних значений, полученных в различных тестах.
    
    На выходе будет ОДИН 3D-рисунок для каждой пары (measure_name, p_within),
    где на одном графике показываем:
      - поверхность для measure_name
      - поверхность для 'random'
    с разными цветами/прозрачностями.
    """

    import csv
    os.makedirs(directory_path, exist_ok=True)
    print(spike_counts_second_cluster_for_input)
    print(spike_counts_second_cluster)
    ...
    with open(avg_csv_filename, 'w', newline='') as csvfile:
        ...
        writer.writerow(['I0_value', 'measure_name', 'p_within', 'p_input', 'p_between', 'mean_spikes', 'std_spikes'])

        # Проверяем, есть ли вообще данные для данного I0_value
        ...
        # measure_names — список, какие метрики мы хотим отобразить
        if measure_names is None:
            measure_names = measure_names_in_data
        
        # Чтобы на каждом графике было две поверхности: "основная" + "random",
        # нам нужно найти measure_name и 'random' в данных.
        # Делаем цикл только по тем measure_name, которые не "random", 
        # поскольку "random" будет отображаться как вторая поверхность.
        for measure_name in measure_names:
            if measure_name == 'random':
                continue
            ...
            # Извлекаем словари для measure_name и random
            ...
            for p_within_str in data_measure.keys():
                ...
                # Для надёжности берём пересечение p_input
                ...
                # Извлекаем список p_between из первого p_input
                ...
                # Аналогично ищем пересечение p_between
                ...
                # Подготовим Z-матрицы для measure_name и random
                # По оси: i -> p_input, j -> p_between
                ...
                # Извлекаем полный список тестовых данных
                ...
                for i, p_inp_str in enumerate(common_p_inputs):
                    # Достаём dict p_between -> mean_spikes
                    ...
                    for j, p_btw in enumerate(p_between_list_float):
                        # Используем p_btw как числовой ключ, а для записи в CSV оставляем строковое представление
                        p_btw_str = f"{p_btw:.2f}"

                        mean_spikes_measure = p_btw_dict_measure.get(p_btw, 0.0)
                        mean_spikes_random  = p_btw_dict_random.get(p_btw, 0.0)
                        Z_measure[i, j] = float(mean_spikes_measure)
                        Z_random[i, j]  = float(mean_spikes_random)
                        ...
                        writer.writerow([
                            ...
                            mean_spikes_measure, stdev_value_measure
                        ])
                        writer.writerow([
                            I0_value, 'random', p_within_str, p_inp_str, p_btw_str,
                            mean_spikes_random, stdev_value_random
                        ])

                # Построение 3D-графика: две поверхности (measure_name и random)
                ...
                # Первая поверхность (measure_name)
                surf_measure = ax.plot_surface(
                    p_input_mesh, p_between_mesh, Z_measure,
                    cmap='magma', edgecolor='none', zorder=1, vmin=0, vmax=max_rate/Hz, 
                    alpha=0.8
                )
                # Вторая поверхность (random)
                surf_random = ax.plot_surface(
                    p_input_mesh, p_between_mesh, Z_random,
                    cmap='viridis', edgecolor='none', zorder=1, vmin=0, vmax=max_rate/Hz, 
                    alpha=0.6
                )

                # Отрисовка error bars для measure_name
                ...
                # Отрисовка error bars для random
                ...
                filename = os.path.join(
                    directory_path,
                    f'3D_two_surfaces_{measure_name}_vs_random_'
                    f'I0_{I0_value}_p_within_{p_within_str}_Time_{current_time}ms.png'
                )
...
# После завершения цикла по measure_name – если имеются результаты для заданных условий, строим общий график
    if subplot_results:
        n_measures = len(subplot_results)
        fig, axes = plt.subplots(1, n_measures, figsize=(5 * n_measures, 8))
        # Если только одна мера – преобразуем axes в список
        if n_measures == 1:
            axes = [axes]
        for ax, (measure, params) in zip(axes, subplot_results.items()):
            W, M, p_in_measured, p_out_measured, percent_central = params
            # Построение connectivity-графика для данной меры
            ...
            # Выбор делений осей (позиции) и соответствующая корректировка меток: от 1 до n_neurons
            ticks = [0, n_neurons // 2, n_neurons - 1]
            ax.set_xticks(ticks)
            ax.set_yticks(ticks)
            
            # Формирование подписей для осей с корректировкой на сдвиг (+1)
            x_labels = [f'{i+1}' if i in highlighted_neurons else f'{i+1}' for i in ticks]
            y_labels = [f'{i+1}' if i in highlighted_neurons else f'{i+1}' for i in ticks]
            ax.set_xticklabels(x_labels)
            ax.set_yticklabels(y_labels)
            
            ax.tick_params(axis='both', which='major', labelsize=10)
        # Формирование имени файла с учетом значений p_input и p_between
        # (так как оба условия могут быть выполнены, имя можно сформировать с объединением)
...
def sim(p_within, p_between, J, J2, refractory_period, sim_time, plotting_flags,
        ...
        I0_value, oscillation_frequency, use_stdp, time_window_size, measure_names, boost_factor_list,
    ...
    centrality = print_centrality(C_total_matrix, N1, n_neurons, p_input, boost_factor, measure_name=measure_name, direct_1_2=True)
    ...
    return avg_neuron_spikes_cluster2_list, time_window_centers, C_total, spike_indices, centrality, p_in_measured, p_out_measured, max_rate, W, M, percent_central
                        ...
                        for measure_name in measure_names:
                            ...
                            for p_between in p_between_values:
                                ...
                                for test_num in range(num_tests):
                                    ...
                                    if test_num == num_tests - 1 and ((p_input == 0.10 and p_between == 0.05) or (p_input == 0.20 and p_between == 0.15)):
                                ...
                                sc_input[p_input_str][p_between] = float(mean_spikes)
