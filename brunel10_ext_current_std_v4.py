        ...
        for V0_value in V0_values:
            for use_stdp in use_stdp_values:
                ...
                for p_within_idx, p_within in enumerate(p_within_values):
                    ...
                    if images:
                        imageio.mimsave(f'results_ext_test1/gif_exc_V0_{V0_value}freq_{oscillation_frequency}_STDP_{"On" if use_stdp else "Off"}_p_within_{p_within:.2f}_Time_{current_time}ms_Bin_{time_window_size}ms.gif', images, duration=3000, loop=0)
                ...
                plt.savefig(f'results_ext_test1/avg_spike_dependency_ext_V0_{V0_value}mV_freq_{oscillation_frequency}Hz_STDP_{"On" if use_stdp else "Off"}_Time_{current_time}ms_Bin_{time_window_size}ms.png')
                plt.close(fig_spike_dependency)

                # Сохраняем данные в CSV
                csv_filename = f'results_ext_test1/avg_spike_dependency_ext_data_V0_{V0_value}mV_freq_{oscillation_frequency}Hz_STDP_{"On" if use_stdp else "Off"}_Time_{current_time}ms_Bin_{time_window_size}ms.csv'
                ...
                for p_within in p_within_values:
                    ...
                    if len(p_between_list) > 0:
                        ...
                        plt.savefig(f'results_ext_test1/3D_plot_V0_{V0_value}mV_freq_{oscillation_frequency}Hz_STDP_{"On" if use_stdp else "Off"}_p_within_{round(p_within, 2)}_Time_{current_time}ms_Bin_{time_window_size}ms.png')
                        # plt.close(fig_3d)
                        plt.show()
