...
t_range = [0, 1000]
rate_range = [0, 200]
oscillation_frequencies = [20, 30, 40]  
use_stdp_values = [False, True]  
# V0_values = np.arange(100, 500, 100)
V0_values = [200, 300]
        ...
        for V0_value in V0_values:
            for use_stdp in use_stdp_values:
                ...
                for p_within_idx, p_within in enumerate(p_within_values):
                    ...
                    if images:
                        imageio.mimsave(f'results/gif_exc_V0_{V0_value}freq_{oscillation_frequency}_STDP_{"On" if use_stdp else "Off"}_p_within_{p_within:.2f}_Time_{current_time}ms.gif', images, duration=3000, loop=0)
                ...
                plt.savefig(f'results/average_spike_dependency_V0_{V0_value}mV_freq_{oscillation_frequency}Hz_STDP_{"On" if use_stdp else "Off"}_Time_{current_time}ms.png')
